...
# Base warm-pool size stays LEAN — a plain launch shouldn't pay for 8 sandboxes it
# won't use. The Incident Desk is the only consumer that NEEDS the full agent-0..7
# pool (its backend reuses agent-0..7 for coordinator + investigators + synthesizer
# + sender — a missing one, e.g. agent-5 the synthesizer, hangs a run). So below we
# FORCE FLEET_SIZE=8 when ENABLE_INCIDENT_DESK=true and the user didn't set it
# explicitly. Detect an explicit override BEFORE applying the default.
FLEET_SIZE_WAS_SET=0; [[ -n "${FLEET_SIZE:-}" ]] && FLEET_SIZE_WAS_SET=1
...
# Incident Desk (optional). The full app stack (research-desk backend, demo-shop,
# Gitea, Mailpit, ArgoCD + the FAULT GitOps catalog) is deployed by the
# 18-incident-desk Ansible role and gated on incident_desk_enabled. Default OFF —
# it's heavy and needs the warm agent-0..7 fleet + SSO.
ENABLE_INCIDENT_DESK="${ENABLE_INCIDENT_DESK:-false}"
# The desk reuses agent-0..7, so force the full warm pool when it's on AND the user
# didn't pin FLEET_SIZE themselves (an explicit FLEET_SIZE always wins).
if [[ "$ENABLE_INCIDENT_DESK" == "true" && "$FLEET_SIZE_WAS_SET" -eq 0 ]]; then
  FLEET_SIZE=8
fi
...
if [[ "$ENABLE_SSO" == "true" ]]; then
  ...
  # Incident Desk: the gitea realm-client secret + the gitea admin (gitops) password.
  # Stable across runs (the Keycloak client + the Gitea account must keep them).
  GITEA_CLIENT_SECRET="${GITEA_CLIENT_SECRET:-$(gen)}"
  GITEA_ADMIN_PASSWORD="${GITEA_ADMIN_PASSWORD:-$(gen)}"
  cat > "$SSO_CACHE" <<EOF
...
# Incident Desk: the gitea Keycloak client secret + the Gitea admin (gitops)
# password. Consumed by the keycloak realm template + the 18-incident-desk role.
gitea_client_secret: "${GITEA_CLIENT_SECRET}"
gitea_admin_password: "${GITEA_ADMIN_PASSWORD}"
EOF
)"
...
# Single-VM launchable toggles (mirrors .env). These are explicit so .env is the
# source of truth — they OVERRIDE the group_vars defaults (incl. any default that
# is itself derived from incident_desk_enabled).
gvisor_enabled: ${ENABLE_GVISOR}
kyverno_enabled: ${ENABLE_KYVERNO}
monitoring_enabled: ${ENABLE_MONITORING}
sandboxes_enabled: ${CREATE_FLEET}
# FLEET_SIZE is already forced to 8 above when ENABLE_INCIDENT_DESK=true (unless the
# user pinned it), and stays at the lean base (5) otherwise.
fleet_size: ${FLEET_SIZE}
gateway_nodeport: ${GATEWAY_NODEPORT}
# Incident Desk: deploys the full stack via the 18-incident-desk role + enables its
# Envoy routes (/argocd, /mailpit) and the GitOps seed. group_vars derives
# incident_desk_routes_enabled from this when not set explicitly.
incident_desk_enabled: ${ENABLE_INCIDENT_DESK}
${SSO_SECRETS_BLOCK}
EOF
chmod 600 "$SECRETS"
...
# ---- Incident Desk GitOps catalog seed (safety net) ------------------------
# The 18-incident-desk Ansible role already deploys the whole desk AND runs this
# same seed as its last step. This post-playbook call is a best-effort safety net
# for a TARGETED re-run (ANSIBLE_TAGS that skipped the desk role) where Gitea +
# ArgoCD already exist but the catalog drifted. The script self-skips if
# Gitea/ArgoCD or the gitea-token secret aren't present, and is idempotent (skips
# if already seeded). A failure here must NOT fail the provision.
if [[ "${ENABLE_INCIDENT_DESK}" == "true" ]]; then
  log "Reconciling the Incident Desk GitOps catalog (Gitea + ArgoCD; no-op if the role already seeded it)"
  KUBECONFIG=/etc/rancher/k3s/k3s.yaml "$ROOT/scripts/seed-gitops-catalog.sh" \
